...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `Video trimmed: ${video._id} -> ${trimmedVideo._id} by user ${req.user.id}`
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `Video cropped: ${video._id} -> ${croppedVideo._id} by user ${req.user.id}`
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      logger.info(
        `Video split: ${video._id} -> ${firstVideo._id}, ${secondVideo._id} by user ${req.user.id}`
      );
    ...
    try {
      ...
      switch (effectType) {
        case "transition":
          ...
          break;
        case "background":
          result = await videoProcessor.processBackground(
            video.filePath,
            parameters
          );
          break;
      ...
      logger.info(
        `Effect applied: ${effectType} to ${video._id} -> ${effectVideo._id} by user ${req.user.id}`
      );
...
router.use((req, res, next) => {
  logger.info(`Video Edit Route - ${req.method} ${req.path}`);
