...
// Inline constants (previously from ../constants)
const UI_CONFIG = {
  BREAKPOINTS: {
    SM: 640,
    MD: 768,
...
export const useUI = () => {
  ...
  const handleResize = useCallback(() => {
    ...
    setIsTablet(
      width >= UI_CONFIG.BREAKPOINTS.MD && width < UI_CONFIG.BREAKPOINTS.LG
    );
    ...
    setLayout((prevLayout) => ({
      ...prevLayout,
      screenSize:
        width < UI_CONFIG.BREAKPOINTS.MD
          ? "mobile"
          : width < UI_CONFIG.BREAKPOINTS.LG
          ? "tablet"
          : "desktop",
  ...
  useEffect(() => {
    ...
    const savedWorkspace = storage.get(
      STORAGE_KEYS.WORKSPACE_LAYOUT,
      workspace
    );
