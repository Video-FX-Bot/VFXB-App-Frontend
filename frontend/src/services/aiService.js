...
class AIService {
  ...
  intentPatterns = {
    ...
    add_text: [/add.*text/i, /title/i, /heading/i, /label/i],
    ...
    // Export and sharing
    export_video: [/export/i, /download/i, /save.*video/i, /render/i],
    share_video: [/share/i, /publish/i, /upload.*to/i],
  };
  ...
  extractParameters(message, intent) {
    ...
    if (timeMatch) {
      ...
      params.duration =
        unit === "minute" ? value * 60 : unit === "hour" ? value * 3600 : value;
    }
    ...
    // Position parameters
    if (
      normalizedMessage.includes("beginning") ||
      normalizedMessage.includes("start")
    ) {
      params.position = "start";
    }
    if (
      normalizedMessage.includes("end") ||
      normalizedMessage.includes("ending")
    ) {
      params.position = "end";
    }
  ...
  async generateResponse(intentResult, parameters, context) {
    ...
    switch (intent) {
      case "audio_enhance":
        return {
          content:
            "I'll enhance the audio quality of your video. This will improve clarity and reduce background noise.",
          actions: [
            {
              label: "Apply Audio Enhancement",
              onClick: () =>
                this.executeVideoOperation("audio_enhance", parameters),
            },
            {
      ...
      case "audio_remove_noise":
        return {
          content:
            "I'll remove background noise from your video to make the audio cleaner and more professional.",
          actions: [
            {
              label: "Remove Background Noise",
              onClick: () =>
                this.executeVideoOperation("noise_reduction", parameters),
            },
            {
      ...
      case "video_trim":
        return {
          content:
            "I can help you trim your video. Would you like to specify the start and end times, or should I help you select the portion to keep?",
          actions: [
            {
      ...
      case "add_subtitles":
        return {
          content:
            "I'll add subtitles to your video. I can either auto-generate them from the audio or you can upload a subtitle file.",
          actions: [
            {
              label: "Auto-generate Subtitles",
              onClick: () =>
                this.executeVideoOperation("auto_subtitles", parameters),
            },
            {
      ...
      case "color_correction":
        return {
          content:
            "I'll help you adjust the colors in your video. This includes brightness, contrast, saturation, and color balance.",
          actions: [
            {
              label: "Auto Color Correction",
              onClick: () =>
                this.executeVideoOperation("auto_color_correct", parameters),
            },
            {
  ...
  async executeVideoOperation(operation, parameters) {
    try {
      ...
      const result = await apiService.processVideo(
