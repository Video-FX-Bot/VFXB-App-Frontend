...
class ApiService {
  ...
  // Video effect endpoints
  async applyVideoEffect(videoId, effect, parameters) {
    console.log("Applying video effect:", { videoId, effect, parameters });
    const endpoint = `/video-edit/${videoId}/effect`;
    console.log("Request URL:", `${this.baseURL}${endpoint}`);
    return this.request(endpoint, {
      method: "POST",
      body: JSON.stringify({ effect, parameters }),
    });
  }
  ...
  async uploadVideo(file, onProgress) {
    ...
    // Add description and visibility with defaults
      ...
      xhr.addEventListener("load", () => {
        try {
          ...
          if (xhr.status >= 200 && xhr.status < 300 && response.success) {
            ...
            const errorMessage =
              response.message || response.error || xhr.statusText;
            console.error("Upload error details:", response); // Debug log
            reject(new Error(`Upload failed: ${errorMessage}`));
          }
        } catch (error) {
          console.error("Error parsing response:", error, xhr.responseText); // Debug log
          reject(
